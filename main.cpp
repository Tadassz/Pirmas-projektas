...
void writeToFile_list(std::list<Studentas>& studentai_list, std::ostream& out) {
    out << std::left
        << std::setw(20) << "Pavarde"
        << std::setw(15) << "Vardas"
    ...
    for (auto & d : studentai_list)
    {
        out << std::left
            << std::setw(20) << d.pavarde
            << std::setw(15) << d.vardas
            << std::setw(15) << std::fixed << std::setprecision(2) << d.galutinisVid
            << std::setw(15) << std::fixed << std::setprecision(2) << d.galutinisMed
            << "\n";
    }
}
...
void sort_vector(std::vector<Studentas> &studentai, std::vector<Studentas> &kietiakai, std::vector<Studentas> &vargsiukai) {
    ...
    std::sort(studentai.begin(), studentai.end(), compareTwoStudents);
    for (auto & i : studentai)
    {
...
{
    studentai_list.sort(compareTwoStudents);
    for(auto stud : studentai_list) {
        if (stud.galutinisVid >= 5.00) {
            kietiakai_list.push_back(stud);
        }
        else {
            vargsiukai_list.push_back(stud);
        }
    }
}
...
std::list<Studentas> avgAndMed_list(std::list<Studentas> studentai_list) {
    ...
    for (auto &student: studentai_list) {
        double avg = 0;
        for (auto stud_grade: student.nd) {
            avg += stud_grade;
        }
        avg /= student.nd.size();
...
bool compareTwoStudents(Studentas a, Studentas b)
{
    ...
    } else return a.galutinisVid > b.galutinisVid;
}
